...
def whichgen(command, path=None, verbose=0, exts=None):
    ...
    # Absolute paths in Windows should be checked without any issue.
    if sys.platform.startswith("win") and command[0:2].lower() == "c:":
        for ext in ['']+exts:
            absName = os.path.normpath(command+ext)
            if os.path.isfile(absName):
                match = _cull((absName, "from absolute directory"), matches, verbose)
                if match:
                    if verbose:
                        yield match
                    else:
